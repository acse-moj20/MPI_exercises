/*/*
 Lecture 4 Exercise 3 : Creating temporary MPI Datatype for boundaries of a 2D domain
 ====================================================================================

Do the transfer as a single communication for each process. 
On the zero process you need to create temporary variables for the transfer of the data to each of the processes. 
This is because the data for each processes will be randomly scattered within the list of data created on processor zero. 
On the other processes you will need to use a probe to determine how many particles they are to receive. 
You will not need to create temporary types on these processes as you can store them as continuous memory and 
you can use a blocking receive as you are only receiving a single piece of information.
...
int main(int argc, char* argv[])
{
	...
	if (id == 0)
	{
		...
		for (int i = 0; i < num_particle_total; i++)
		{
			for (int j = 0; j < 2; j++)
			{
				full_particle_list[i].x[j] = random_no() * domain_size[j];
				full_particle_list[i].v[j] = random_no() * max_vel;
			}
		}
	}
